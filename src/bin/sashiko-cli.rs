...
/// Orders stage rows the way the review runs them.
///
/// Stages are identified by name, so the order has to come from the workflow's
/// own table rather than from the identifier. An entry with no stage is the
/// patch's own row and sorts above the stages it covers; a name the table does
/// not know sorts last rather than vanishing.
fn stage_sort_key(stage: &Value) -> usize {
    match stage.as_str() {
        Some(name) => sashiko::worker::kernel_workflow::stage_order(name).saturating_add(1),
        None => 0,
    }
}
...
fn stage_row(entry: &Value, live: bool) -> String {
    ...
    // Stages are named, not numbered. Reading this as an integer left every
    // stage falling back to the patch's own label, so a table of concurrent
    // stages read as a column of identical rows.
    let stage = entry
        .get("stage")
        .and_then(|s| s.as_str())
        .map(|s| s.to_string())
    ...
    if !live {
        ...
        return format!("    {:<20} {:>10}  {:>12}  {}", stage, dash, dash, state);
    }
    ...
    for patch in patches {
        ...
        match (live_entries, recorded) {
            ...
            (Some(entries), _) => {
                ...
                // The patch's own entry has no stage and sorts first, above the
                // stages it covers. The rest go in the order the review runs
                // them, which a name cannot supply on its own.
                entries.sort_by_key(|e| stage_sort_key(&e["stage"]));
                println!(
                    "    {:<20} {:>10}  {:>12}  STATE",
                    "STAGE", "ELAPSED", "TURNS"
                );
            ...
            (None, Some(stages)) => {
                ...
                stages.sort_by_key(|s| stage_sort_key(&s["stage"]));
